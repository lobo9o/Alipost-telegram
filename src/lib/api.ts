...
// ── Telegram Monitor ─────────────────────────────────────────────────────────
export interface TgMonitorChannel { id: string; channel: string; active: boolean; auto_publish: boolean; dest_channel?: string | null; force_errore?: boolean; platform_filter?: string; }
export const tgMonitorApi = {
  ...
  updateChannel: (id: string, data: Partial<Pick<TgMonitorChannel, 'auto_publish' | 'active' | 'force_errore' | 'platform_filter'>> & { dest_channel?: string | null }) => req<{ ok: boolean }>('PATCH', `/api/tg-monitor/channels/${id}`, data),
};
