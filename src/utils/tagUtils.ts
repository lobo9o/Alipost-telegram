...
function computedTags(post: CreatedPost): Record<string, string> {
  ...
  return {
    ...
    '{prezzo}':         `${valuta}${post.discountedPrice.toFixed(2)}`,
    '{prezzo_scontato}': `${valuta}${post.discountedPrice.toFixed(2)}`,
    '{oldprezzo}':      `${valuta}${post.originalPrice.toFixed(2)}`,
...
export function resolvePostTags(template: string, post: CreatedPost, tags: Tag[]): string {
  ...
  // Gestione blocchi condizionali {_ ... _}: nasconde il blocco se un tag al suo interno è vuoto
  let result = template.replace(/\{_([\s\S]*?)_\}/g, (_, inner) => {
    let hasEmpty = false;
    for (const [tag, val] of Object.entries(builtIn)) {
